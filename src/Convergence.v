...
Definition conv {Var : Set} (x : Term Var) : Prop :=
  exists y, probe (close x) y /\ pi y TOP.

Lemma conv_close (Var : Set) (x : Term Var) : conv (close x) <-> conv x.
Proof.
  unfold conv; term_simpl; tauto.
Qed.
...
Inductive prob {Var : Set} : Term Var -> Prop :=
  | prob_top : prob TOP
  | prob_bot : prob BOT
  | prob_r p q : prob p -> prob q -> prob (p (+) q).
Hint Constructors prob.

Definition pconv {Var : Set} (x : Term Var) (p : Term Empty_set) : Prop :=
...
Lemma conv_top (Var : Set) : conv (TOP : Term Var).
Proof.
...
Inductive heads {Var : Set} : relation (Term Var) :=
...
Lemma heads_close (Var : Set) (h x : Term Var) :
  heads h x -> heads (close h) (close x).
Proof.
  intro H; induction H; simpl; compute; heads.
Qed.

Lemma heads_probe (Var : Set) (h x y : Term Var) :
...
Lemma heads_beta_bot (Var : Set) (x y : Term Var) :
  beta x y -> heads BOT x -> heads BOT y.
Proof.
  intros Ht; induction Ht; intros; heads.
Qed.

Lemma heads_pi_bot (Var : Set) (x y : Term Var) :
  pi x y -> heads BOT x -> heads BOT y.
Proof.
  intros Ht; induction Ht; intros; heads.
Qed.

Lemma not_conv_heads_bot (Var : Set) (x : Term Var) : heads BOT x -> ~ conv x.
Proof.
  intros H [y [xy yt]].
...
Lemma not_conv_bot (Var : Set) : ~ conv (BOT : Term Var).
Proof.
